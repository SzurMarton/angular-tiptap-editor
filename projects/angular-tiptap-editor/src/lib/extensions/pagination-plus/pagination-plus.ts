...
const defaultOptions: PaginationPlusOptions = {
  ...
  pageGap: 50,
  pageGapBorderSize: 1,
  pageBreakBackground: "var(--app-bg, var(--ate-background, var(--ate-surface, #ffffff)))",
  footerRight: "{page}",
  footerLeft: "",
  headerRight: "",
  headerLeft: "",
...
const PAGINATION_STYLE_SELECTOR = "style[data-rm-pagination-style]";
const PAGE_FRAME_PADDING_PX = 12;

const cleanupPaginationDom = (targetNode: HTMLElement) => {
  targetNode.classList.remove("rm-with-pagination");
  targetNode.style.removeProperty("border");
  targetNode.style.removeProperty("box-sizing");
  targetNode.style.removeProperty("max-width");
  targetNode.style.removeProperty("margin-left");
  targetNode.style.removeProperty("margin-right");
  targetNode.style.removeProperty("padding-left");
  targetNode.style.removeProperty("padding-right");
  targetNode.style.removeProperty("padding-top");
  targetNode.style.removeProperty("padding-bottom");
  targetNode.style.removeProperty("width");
  targetNode.style.removeProperty("min-height");

  // Remove pagination CSS variables generated by this extension.
  for (let i = targetNode.style.length - 1; i >= 0; i--) {
    const propertyName = targetNode.style.item(i);
    if (propertyName.startsWith("--rm-")) {
      targetNode.style.removeProperty(propertyName);
    }
  }

  targetNode.querySelectorAll("[data-rm-pagination], .rm-first-page-header").forEach(el => {
    el.remove();
  });
};
...
export const PaginationPlus = Extension.create<PaginationPlusOptions, PaginationPlusStorage>({
  ...
  onCreate() {
    ...
    targetNode.style.boxSizing = "content-box";
    targetNode.style.maxWidth = "100%";
    targetNode.style.marginLeft = "auto";
    targetNode.style.marginRight = "auto";
    targetNode.style.paddingTop = `${PAGE_FRAME_PADDING_PX}px`;
    targetNode.style.paddingBottom = `${PAGE_FRAME_PADDING_PX}px`;
  ...
  onDestroy() {
    ...
    if (styleElement && styleElement.parentNode) {
      ...
      document.querySelectorAll(PAGINATION_STYLE_SELECTOR).forEach(el => el.remove());
    }

    cleanupPaginationDom(this.editor.view.dom);
  },
  ...
  const pageWidget = Decoration.widget(
    ...
    view => {
      ...
      const pageBreakDefinition = (
        ...
        pageSpace.style.setProperty("width", "100%", "important");
        pageSpace.style.left = "0";
