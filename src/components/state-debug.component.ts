...
@Component({
  ...
  styles: [`
    ...
    .pill-key { color: var(--text-secondary); font-weight: 400; font-size: 0.8rem; }
    .pill-val { color: var(--primary-color); font-weight: 700; }
    ...
    .path-sep { color: var(--app-border); font-weight: 300; }
    ...
    .mark-tag .material-symbols-outlined { font-size: 1.1rem; }
...
export class StateDebugComponent {
  ...
  readonly hierarchy = computed(() => {
    ...
    if ((s.nodes as any).taskList || (s.nodes as any).taskItem) {
      items.push({ name: (s.nodes as any).taskList ? 'taskList' : 'taskItem', active: true });
    }
  ...
  readonly activeMarks = computed(() => {
    ...
    return Object.keys(m)
      .map(key => ({ name: key, value: (m as any)[key] }))
      .filter(mark =>
        mark.value === true ||
        (typeof mark.value === 'string' && mark.value.length > 0)
      );
  });
